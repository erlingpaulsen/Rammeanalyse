...
% Plotfig lager et plot av den aktuelle konstruksjonen og viser nummerering
% av knutepunkter og elementer, slik at det blir enklere aa analyse resultatet
...
% nelem: Antall elementer
    ...
    % Begynner med aa legge alle punkter inn i en matrise
    ...
    %Plotter en linje mellom punkter der det gaar et element.
    ...
    for j = 1:npunkt
        ...
        text(punkt(j,1)+(dx/20), punkt(j,2)+(dy/11), num2str(j), 'Color', 'b');
    end
    
    for k = 1:nelem
        if ((m(k, 1)+m(k, 3))/2 == m(k, 1))
            text(((m(k, 1) + m(k, 3))/2)-(dx / 13),...
                ((m(k, 2)+m(k, 4))/2), num2str(k), 'Color', 'r');
        else
            text(((m(k, 1)+m(k, 3))/2),...
            ((m(k, 2) + m(k, 4))/2) - (dy / 15), num2str(k), 'Color', 'r');
        end
    end
    
    % Lager dimensjonene av plottet. Bruker relative
    % verdier slik at koden fungerer selv for smaa eller
    % ekstremt store konstruksjoner.
    xlim([xmin - ((dx/4)) xmax + ((dx/4))]);
    ylim([ymin - ((dy/4)) ymax + ((dy/2) + 1)]);
    
    % Litt forklarende tekst paa plottet til slutt
    title('Nummerering av knutepunkt og elementer');
    text(xmin, ymax+((dy/2.5)), 'Blaa: knutepunktnr.');
    text(xmax-((dx/2)), ymax+((dy/2.5)), 'x: fastholdt mot rotasjon');
    text(xmin, ymax+((dy/3.5)), 'Roede: elementnr.');
