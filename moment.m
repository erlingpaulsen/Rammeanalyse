function fim = moment(npunkt, punkt, nelem, elem, nlast,...
    last, elementlengder)
...
% fim: Vektor med fastinnspenningsmomentene
    ...
    for i = 1 : nlast;
        ...
        if lasttype == 0;
            lastvinkel = last(i, 5);
            temp1 = (- lastint1*cosd(lastvinkel) *...
                lastavstand * (el-lastavstand)^2)/(el^2);
            temp2 = (lastint1 *cosd(lastvinkel)*...
                lastavstand * (el-lastavstand)^2)/(el^2);
        
        % Regner ut fastinnspenningsmomentet til en fordelt last
        elseif lasttype == 1;
            lastint2 = last(i, 5);
            temp1 = ((-1/20) * lastint1 * (el)^2)...
                + ((-1/30) * lastint2 * (el)^2);
            temp2 = ((1/30) * lastint1 * (el)^2)...
                + ((1/20) * lastint2 * (el)^2);
        else
